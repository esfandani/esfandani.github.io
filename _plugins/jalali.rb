...
# https://github.com/aziz/jalalidate
class JalaliDate < Time
  ...
  def <=>(other)
    if other.respond_to? :to_g
      to_g <=> other.to_g
    else
      to_g <=> JalaliDate.new(other).to_g
    end
  end
  ...
  def strftime(format_str = '%Y/%m/%d')
    format_str.gsub(/%{2}/, "SUBSTITUTION_MARKER").
...
class JalaliDate
  ...
  def strftime(format_str = '%Y/%m/%d')
    strftime_ format_str.
      gsub(/%m/, "%02d" % @month).
      gsub(/%_m/, @month.to_s.rjust(2, ' ')).
      gsub(/%-m/, @month.to_s)
  end
  def to_liquid
    self
...
# by https://github.com/nbjahan
class Time
  class << self
    def parse_with_jalali(date, now=self.now)
      # TODO: valid until 1499
      date.match(/^(1[34]\d\d).?(\d{1,2}).?(\d{1,2})/) { |m|
        date = date.sub(m[0],JalaliDate.new(m[1].to_i, m[2].to_i, m[3].to_i).to_g.to_s)
      }
      if block_given?
        JalaliDate.new(parse_without_jalali(date, now, &Proc.new))
      else
        JalaliDate.new(parse_without_jalali(date,now))
      end
    end
    alias parse_without_jalali parse
    alias parse parse_with_jalali
  end
end
